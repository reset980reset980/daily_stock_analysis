...
function formatCorporateActionLabel(value: PortfolioCorporateActionType): string {
  return value === 'cash_dividend' ? '현금 배당' : '액면분할/병합 조정';
}

function formatBrokerLabel(value: string, displayName?: string): string {
  const normalized = (displayName || '').trim();
  if (normalized === '华泰') return 'huatai (Huatai)';
  if (normalized === '中信') return 'citic (CITIC)';
  if (normalized === '招商') return 'cmb (CMB)';
  if (displayName && normalized) return `${value} (${normalized})`;
...
function buildFxRefreshFeedback(data: PortfolioFxRefreshResponse): FxRefreshFeedback {
  if (data.refreshEnabled === false) {
    return {
      tone: 'neutral',
      text: '온라인 환율 자동 갱신이 비활성화되어 있습니다.',
    };
  }
  ...
  if (data.updatedCount > 0 && data.staleCount === 0 && data.errorCount === 0) {
    return {
      tone: 'success',
      text: `환율 갱신이 완료되었습니다. 총 ${data.updatedCount}개를 업데이트했습니다.`,
    };
  }
  ...
  if (data.staleCount > 0) {
    return {
      tone: 'warning',
      text: `갱신을 시도했지만 일부 통화 쌍은 여전히 지연/대체 환율을 사용합니다. ${summary}`,
    };
  }

  return {
    tone: 'warning',
    text: `온라인 환율 갱신이 완전히 성공하지는 않았습니다. ${summary}`,
  };
}
...
const PortfolioPage: React.FC = () => {
  ...
  const displayCurrency = hasAccounts ? (snapshot?.currency || 'KRW') : 'KRW';
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning('브로커 목록 API가 비어 있어 기본 브로커 목록(Huatai/CITIC/CMB)으로 대체했습니다.');
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || '위험 분석 데이터를 가져오지 못해 스냅샷만 표시합니다.');
      }
    } catch (err) {
      const parsed = getParsedApiError(err);
      setSnapshot(null);
      setRisk(null);
      if (!(accounts.length === 0 && parsed.message.includes('DatabaseManager'))) {
        setError(parsed);
      }
    } finally {
      setIsLoading(false);
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning('오른쪽 위에서 실제 계정을 먼저 선택한 뒤 입력 또는 CSV 가져오기를 진행하세요.');
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning('오른쪽 위에서 실제 계정을 먼저 선택한 뒤 삭제 또는 정정 작업을 진행하세요.');
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    if (!name) {
      setAccountCreateError('계정 이름은 필수입니다.');
    ...
    try {
      ...
      setAccountCreateSuccess('계정을 만들었고 새 계정으로 자동 전환했습니다.');
    } catch (err) {
      const parsed = getParsedApiError(err);
      setAccountCreateError(parsed.message || '계정 생성에 실패했습니다. 잠시 후 다시 시도해 주세요.');
      setAccountCreateSuccess(null);
    } finally {
    ...
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      <section className="space-y-3">
        <div className="space-y-2">
          ...
          <p className="text-xs md:text-sm text-secondary">
            포트폴리오 스냅샷, 수기 입력, CSV 가져오기, 위험 분석을 지원합니다. 전체 보기와 단일 계정 전환도 가능합니다.
          </p>
        </div>
        {hasAccounts ? (
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-3">
            <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_220px_280px] gap-2 items-end">
              ...
              <div>
                <p className="text-xs text-secondary mb-1">成本요약 기준</p>
                <select
                  value={costMethod}
              ...
              <div className="flex gap-2">
                ...
                >
                  {showCreateAccount ? '계정 생성 접기' : '새 계정 만들기'}
                </button>
                <button
                  type="button"
          ...
          <InlineAlert
            ...
            message="아직 사용할 수 있는 계정이 없습니다. 먼저 계정을 만든 뒤 거래 입력이나 CSV 가져오기를 진행하세요."
          />
        )}
      </section>
        ...
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            ...
            {hasAccounts ? (
              ...
              <span className="text-xs text-secondary">생성 후 이 계정으로 자동 전환됩니다</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            ...
            >
              <option value="us">시장: 미국 주식 (USD)</option>
              <option value="cn">시장: 아시아 주식형 (KRW 기준)</option>
              <option value="hk">시장: 홍콩 주식 (HKD)</option>
            </select>
            <button type="submit" className="btn-secondary text-sm" disabled={accountCreating}>
              {accountCreating ? '생성 중...' : '계정 만들기'}
            </button>
          </form>
        </Card>
      ) : null}
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        <Card variant="gradient" padding="md">
          <p className="text-xs text-secondary">총 자산</p>
          <p className="mt-1 text-xl font-semibold text-foreground">{formatMoney(snapshot?.totalEquity, displayCurrency)}</p>
        </Card>
        <Card variant="gradient" padding="md">
          <p className="text-xs text-secondary">총 평가금액</p>
          <p className="mt-1 text-xl font-semibold text-foreground">{formatMoney(snapshot?.totalMarketValue, displayCurrency)}</p>
        </Card>
        <Card variant="gradient" padding="md">
          <p className="text-xs text-secondary">총 현금</p>
          <p className="mt-1 text-xl font-semibold text-foreground">{formatMoney(snapshot?.totalCash, displayCurrency)}</p>
        </Card>
        <Card variant="gradient" padding="md">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card className="xl:col-span-2" padding="md">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-foreground">보유 종목 상세</h2>
            <span className="text-xs text-secondary">총 {positionRows.length}개</span>
          </div>
          {positionRows.length === 0 ? (
            <EmptyState
              title="아직 보유 종목 데이터가 없습니다"
              description="거래를 입력하거나 CSV를 가져오면 계정별 보유 종목 상세가 여기에 표시됩니다."
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="text-xs text-secondary border-b border-white/10">
                  <tr>
                    <th className="text-left py-2 pr-2">계정</th>
                    <th className="text-left py-2 pr-2">종목코드</th>
                    <th className="text-right py-2 pr-2">수량</th>
                    <th className="text-right py-2 pr-2">평균단가</th>
                    <th className="text-right py-2 pr-2">현재가</th>
                    <th className="text-right py-2 pr-2">평가금액</th>
        ...
        <Card padding="md">
          <h2 className="text-sm font-semibold text-foreground mb-3">{concentrationMode === 'sector' ? '行业集中度分布' : '업종 데이터가 아직 없어 종목 집중도로 대신 표시합니다'}</h2>
          {concentrationPieData.length > 0 ? (
            ...
            <EmptyState
              title="집중도 데이터가 아직 없습니다"
              description="위험 분석이 계산되면 업종 또는 종목 기준의 집중도 분포가 여기에 표시됩니다."
              className="border-none bg-transparent px-4 py-10 shadow-none"
            />
          )}
          <div className="mt-3 text-xs text-secondary space-y-1">
            <div>표시 기준: {concentrationMode === 'sector' ? '업종 기준' : '종목 기준 (대체 표시)'}</div>
            <div>섹터 집중 경고: {risk?.sectorConcentration?.alert ? '예' : '아니오'}</div>
            <div>상위 1개 비중: {formatPct(risk?.sectorConcentration?.topWeightPct ?? risk?.concentration?.topWeightPct)}</div>
          </div>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-2">손절 근접 경고</h3>
          <div className="text-xs text-secondary space-y-1">
            <div>발생 건수: {risk?.stopLoss?.triggeredCount ?? 0}</div>
            <div>근접 건수: {risk?.stopLoss?.nearCount ?? 0}</div>
            <div>경고: {risk?.stopLoss?.nearAlert ? '예' : '아니오'}</div>
          </div>
        </Card>
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-2">요약 기준</h3>
          <div className="text-xs text-secondary space-y-1">
            <div>계정 수: {snapshot?.accountCount ?? 0}</div>
            <div>기준 통화: {displayCurrency}</div>
            <div>원가 계산 방식: {(snapshot?.costMethod || costMethod).toUpperCase()}</div>
          </div>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">수기 입력: 거래</h3>
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder="종목 코드 (예: AAPL, 005930)" value={tradeForm.symbol}
            ...
            <p className="text-xs text-secondary">수수료와 세금은 비워두면 0으로 처리됩니다.</p>
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>거래 저장</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">수기 입력: 자금 흐름</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            ...
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={`통화 (선택, 기본값: ${writableAccount?.baseCurrency || '계정 기준 통화'})`} value={cashForm.currency}
              onChange={(e) => setCashForm((prev) => ({ ...prev, currency: e.target.value }))} />
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>자금 흐름 저장</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">수기 입력: 기업 이벤트</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            ...
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>기업 이벤트 저장</button>
          </form>
        </Card>
      </section>
        ...
        <Card padding="md">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">미리보기만 실행 (저장 안 함)</label>
            </div>
            <div className="flex gap-2">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={eventType} onChange={(e) => setEventType(e.target.value as EventType)}>
                ...
                <option value="cash">자금 기록</option>
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder="종목 코드로 필터" value={eventSymbol}
            ...
            <div className="text-[11px] text-secondary">
              {writeBlocked ? '삭제 및 정정은 단일 계정 보기에서만 가능합니다. 먼저 실제 계정을 선택해 주세요.' : '잘못된 기록은 삭제한 뒤 다시 입력할 수 있습니다.'}
            </div>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: `确认삭제 ${item.tradeDate} 的${formatSideLabel(item.side)}流水 ${item.symbol}（数量 ${item.quantity}，价格 ${item.price}）吗？`,
                      })}
                    >
                      삭제
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: `确认삭제 ${item.eventDate} 的资金流水（${formatCashDirectionLabel(item.direction)} ${item.amount} ${item.currency}）吗？`,
                      })}
                    >
                      삭제
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: `确认삭제 ${item.effectiveDate} 的公司行为 ${formatCorporateActionLabel(item.actionType)}（${item.symbol}）吗？`,
                      })}
                    >
                      삭제
                    </button>
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title="기록이 없습니다"
                      description="필터 조건을 바꾸거나 거래/자금/기업 이벤트를 먼저 입력해 주세요."
      ...
      <ConfirmDialog
        ...
        title="삭제错误流水"
        message={pendingDelete?.message || '确认삭제这条流水吗？'}
        confirmText={deleteLoading ? '삭제中...' : '确认삭제'}
        cancelText="取消"
        isDanger
